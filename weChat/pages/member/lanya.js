...
Page({
    ...
    updateDevice:function (devices) {
        let _devices = this.data.devices;
        if(_devices.length > 0) {
            _devices.forEach(function (item) {
                if(devices.deviceId != item.deviceId) {
                    devices.distance = this.createDistance(devices.RSSI);
                    item.rate = 0;
                    _devices.push(devices)
                }
                else {
                    item.distance = this.createDistance(devices.RSSI);
                    item.rate++;
                }
            });
        }
    ...
    createDistance:function (RSSI) {
        let distance = Math.abs(RSSI);
        distance = Math.pow(10,(distance - 59) / (10 * 2.0));
        return distance;
    },
